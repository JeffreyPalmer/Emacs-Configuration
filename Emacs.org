...
** Font Configuration
#+begin_src emacs-lisp
  ; (set-face-attribute 'default nil :font "Jetbrains Mono" :height jpalmer/default-font-size :weight 'thin)
  (set-face-attribute 'default nil :font "Fira Code" :height jpalmer/default-font-size :weight 'light)

  ;; Set the fixed pitch face
  ; (set-face-attribute 'fixed-pitch nil :font "Jetbrains Mono" :height jpalmer/default-font-size :weight 'thin)
  (set-face-attribute 'fixed-pitch nil :font "Fira Code" :height jpalmer/default-font-size :weight 'light)

  ;; Set the variable pitch face
  (set-face-attribute 'variable-pitch nil :font "Fira Sans" :height jpalmer/default-variable-font-size :weight 'light)
...
Try out the Modus themes. These are currently not in use, but they have a lot of flexibility so they may be worth looking into later.
...
#+end_src
** Window layout management via persp-mode
...
Try to configure the built-in typescript tree-sitter-based mode.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (with-eval-after-load 'org-faces
    ...
    (set-face-attribute 'org-document-title nil :font "Fira Sans" :weight 'light :height 1.3)
    (dolist (face '((org-level-1 . 1.25)
                    ...
                    (org-level-7 . 1.1)
                    (org-level-8 . 1.1)))
      (set-face-attribute (car face) nil :font "Fira Sans" :weight 'light :height (cdr face)))
