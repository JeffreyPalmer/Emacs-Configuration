...
This file contains the complete, documented configuration for my
Emacs.

The goal of this work is to be able to generate a fixed, known-good
configuration for emacs from this file. A bunch of the content in this
file is inspired by/taken from the [[https://systemcrafters.net/emacs-from-scratch/][System Crafters]] site/channel.

If this is the first time that a new computer is being configured,
then fonts will need to be installed, as documented [[*Modeline Improvements][here]].

NOTE: In order to configure a package that ships with emacs, use the
following form:

  ~(use-package org :straight (:type built-in))~

NOTE: Use the following approach to conditionally configure based on emacs version:
#+begin_src emacs-lisp :tangle no
  (use-package which-key
    ;; only install for emacs < 30 (use-package also provides an :unless clause)
    :if (version< emacs-version "30"))
#+end_src

TODO: Figure out how to use conditionals for my various operating system versions
...
Include these warnings at the top of both generated files so that I
...
Trying to land on a set of fonts that are easily installable on both
linux and MacOS. Looks like Jetbrains Mono and Lato are a good
starting point.
...
** straight.el
Now straight.el needs to be bootstrapped. This bootstrapping code is
provided directly by the [[https://github.com/radian-software/straight.el][straight.el]] project.
...
I'm not planning to use emacs-plus any longer so I think this can be
removed.

Apparently not, as this caused native compilation to fail. I suppose
you DO need to set up the shell environment.
...
This allows things like buffer cleanup to happen during the day
instead of at midnight, when the computer is not typically on.
...
* Built-In Packages
** Transient
This is a built-in package, but we want the lastest released version, so override the built-in version here.
...
I'm currently testing out some other options, but my previous go-to
theme was "doom-tomorrow-night".

*** Modus Themes (DISABLED)
Try out the Modus themes. Currently not using this because I don't
have time to read a novel to understand how to use it.
...
NOTE: The first time this configuration is loaded, the mode line icons
will need to be installed via =M-x all-the-icons-install-fonts=.
...
This doesn't appear to have any effect in emacs-mac, but it is
documented as the "right thing to do" for emacs-plus, so who knows.
...
The package [[https://github.com/jdtsmith/ultra-scroll][ultra-scroll]] improves the scrolling experience with
emacs-mac. It [[https://github.com/jdtsmith/emacs-mac/issues/47][may get integrated into the main build]], so this may not
...
There are some keybinds that I absolutely hate when operating in a
modern graphical desktop environment. These should be disabled, but
only when in a windowed mode.
...
#+end_src
** Text Scaling
I used to have code to do this, but it turns out that there are
interactive screen scaling commands already in emacs, bound to =C-x
C-+=, =C-x C--=, and =C-x C-0=. Plus, they're interactive in the same way
that Hydra provides, so ultimately this configuration is not needed.

** Navigation
[[https://github.com/abo-abo/avy][Avy]] makes it possible to jump to visible text using a character-based
decision tree.

This is apparently [[https://karthinks.com/software/avy-can-do-anything/][a very powerful package]] that I'm under-using. I
should fix that.
...
*** Vertico
[[https://github.com/minad/vertico][Vertico]] is a new completion UI that integrates with the default emacs
completion system.
...
*** Consult
[[https://github.com/minad/consult][Consult]] is a sister package to vertico and serves as the counsel
equivalent to Ivy.
...
[[https://github.com/minad/corfu][Corfu]] is a new in-buffer completion approach from the person that
created Vertico.
...
#+end_src
** Project Tab Groups
use [[https://github.com/fritzgrabo/project-tab-groups][project-tab-groups]] to isolate buffers by project.
#+begin_src emacs-lisp
...
#+end_src
** Tabspaces
Try out [[https://codeberg.org/mclear-tools/tabspaces][tabspaces]] for managing files in tab contexts.
#+begin_src emacs-lisp :tangle no
  (use-package tabspaces
    ;; use this next line only if you also use straight, otherwise ignore it.
    :straight (:type git :host codeberg :repo "mclear-tools/tabspaces")
    ...
    (tabspaces-todo-file-name "README.md")
    ;; sessions
    (tabspaces-session t)
...
#+end_src
** Window layouts via activities.el [disabled]
Trying out [[https://github.com/alphapapa/activities.el][activities.el]] for window management along with trying out tab bar mode.
#+begin_src emacs-lisp :tangle no
...
This seems like it might work nicely with projectile mode? Let's try
it out.
...
#+end_src
** Window management via persp-mode [disabled]
#+begin_src emacs-lisp :tangle no
  (use-package persp-mode
    :custom
    (persp-mode t)
    (persp-autosave-mode t)
...
Winner mode allows you to easily undo/redo window configuration
changes by pressing <C-c left> or <C-c right>
...
TODO: It would be nice if the documentation showed up in an ephemeral window on the right side of the screen
#+begin_src emacs-lisp
...
There's a lot that can be done using this variable, but I'm not really
trying very hard here. See [[https://protesilaos.com/codelog/2024-02-08-emacs-window-rules-display-buffer-alist/][this blog post]] for an overview.
...
* General Editing
** Hungry Delete
This deletes all whitespace up to the last non-whitespace character
when editing. It can be very handy.
...
This should only be necessary with ~emacs-mac~ as ~emacs-plus~ has its own
process for injecting environment variables.
...
*** Keychain support
This is required to ensure that SSH interaction with GitHub (for
example) is seamless.
...
**** Indent-bars
[[https://github.com/jdtsmith/indent-bars][Indent-bars]] is an alternative to ~highlight-indent-guides~ that behaves
a bit better w.r.t. empty lines.

This is currently disabled because it doesn't work at all with Common
Lisp code. (Going to simply disable this in lisp modes)
...
*** Project Support (via Projectile) [disabled]
TODO: Try out the new built-in support for projects using
project.el. A bunch of stuff might have to change, so that's not
...
*** Project Support (via project.el)
#+begin_src emacs-lisp
  (use-package project)
#+end_src
*** Search with RipGrep
...
Only install this if we're on MacOS. Probably should parameterize some
...
First, ensure that we are [[https://emacs-lsp.github.io/lsp-mode/page/performance/#use-plists-for-deserialization][using plists for deserialization]] in LSP
mode.
#+begin_src emacs-lisp :tangle ~/.config/emacs/early-init.el
...
*** Julia
**** LSP
LSP seems to provide the most advanced Julia environment that I've
found so far that works well in Emacs.

Note to self: For some unknown reason I can't move the ~julia-mode~
declaration into its own block without breaking the linkage between
~julia-lsp~ and ~julia-mode~, and I don't have time to figure it out.
...
**** Julia Snail
~julia-snail~ tries to provide a REPL experience closer to
lisp. Unfortunately this doesn't provide enough information for my
...
*** Common Lisp
**** SLY
I use [[https://github.com/joaotavora/sly][SLY]] - a fork of Slime that has some nice features. Vera renders properly in the Sly REPL, which for some reason I'm not interested in exploring SLIME does not.
...
#+end_src
**** SLIME
Try out the lisp OG instead of Sly. The Vera REPL-based rendering doesn't work without some investigation.
#+begin_src emacs-lisp
  (use-package slime
    :config
...
Now, set up ~info~ so that it can see the ANSI Common Lisp
Specification.
#+begin_src emacs-lisp
  (use-package info
    :config
...
This is a tree-sitter-based approach that is currently disabled.  For
some reason this attempt at hooking into =treesit-auto= doesn't work. I
ended up having to install the WGSL treesitter grammar by hand.
...
#+end_src
** Compilation
Set up the compile package and ensure that compilation output
...
#+end_src
** Snippets
All of these snippets packages are currently disabled because I barely
use them and they were not behaving nicely. I may give tempel a try
again at some point.

*** Yasnippets [disabled]
I think these are being a little flaky but I don't have time to
...
*** tempel [disabled]
I'm giving [[https://github.com/minad/tempel][tempel]] a try, but it's a little odd. At least, I think my
...
;; :tangle ~/.config/emacs/etc/tempel/templates.eld
;; -*- mode: lisp -*-
...
#+end_src
** Beads
Use beads.el to be able to interactively browse and edit a beads repo.
#+begin_src emacs-lisp
  (use-package beads
...
#+end_src
** Support org-capture
This needs to come after we install and configure org, or else emacs
will use the built-in version
#+begin_src emacs-lisp
  (require 'org-protocol)
#+end_src
** Better Fonts
*** Improved bullet formatting [disabled]
I've disabled this for now, because something was causing weird bullet
display issues. I'm testing my configuration without this to see if
it's the culprit.
#+begin_src emacs-lisp :tangle no
...
Turns out that this theme configures TODO faces differently than org,
so configure those here.
#+begin_src emacs-lisp :tangle no
...
** Org Roam
I don't really use this in any meaningful way - I should probably move
the content that I did put into it back into normal org files, or
actually commit to doing something more with it. Being in this halfway
state is not great.
#+begin_src emacs-lisp :tangle no
...
#+end_src
*** Show org markup when inside of a formatted element
Use [[https://github.com/awth13/org-appear][org-appear]] to get this behavior.
#+begin_src emacs-lisp
...
** Org-QL
[[https://github.com/alphapapa/org-ql][org-ql]] is a package that provides a query language for org files, as
well as some advanced search capabilities.

TODO: Figure out how to use this to provide agenda-like views that are
more advanced than my current agenda views (probably not really
necessary, tbh)
#+begin_src emacs-lisp
  (use-package org-ql
...
Trying to get a working configuration of ~org-capture~ without using
~emacs-mac~.
...
Binding this new (in Emacs 30) variable to ~t~ will set the completion
to use ~M-/~ which allows for word completion when desired, but skips
using a popup. Hooray!
...
* LLM Support
This enables the =gptel= package, which is used to allow emacs to
interact with LLM servers.
